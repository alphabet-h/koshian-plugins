...
> **Note**: 本ファイル内の `<PLUGIN_ROOT>` プレースホルダは、**呼び出し元 SKILL.md の Step 0 で確定済みの絶対パス**に置換して使うこと。本ファイル単独で resolve する必要はない。
...
```bash
# 1. plugin scaffold
bash <PLUGIN_ROOT>/skills/skill-creator-forge/scripts/scaffold-plugin.sh \
...
if [ -z "$MARKETPLACE_ROOT" ]; then
  ...
  bash <PLUGIN_ROOT>/skills/skill-creator-forge/scripts/update-marketplace.sh \
  ...
  bash <PLUGIN_ROOT>/skills/skill-creator-forge/scripts/update-marketplace.sh --yes \
    --marketplace-root "$MARKETPLACE_ROOT" \
    --plugin-name "<plugin-name>" \
    --plugin-source "./plugins/<plugin-name>" \
    --plugin-description "<description>"
